...
export function GenerarInforme({ onCancel }) { // onCancel para manejar el botón de cancelar
  ...
  const chartOptions = {
    maintainAspectRatio: false, // Desactiva el mantener la proporción para ajustar el tamaño
    responsive: true,
    aspectRatio: 1.5, // Ajusta el ratio para hacerlo más pequeño
  };
    ...
    <Card className="mt-6">
      <CardBody>
        ...
        <div className="flex gap-4">
          <Button onClick={handleGenerarInforme}>Generar Informe</
          Button> <Button onClick={onCancel} color="red">Cancelar</Button> {/* Botón de cancelar */} </div>
          {informeGenerado && (
      <Button onClick={handleDescargarExcel} className="mt-4">Descargar Excel</Button>
    )}
